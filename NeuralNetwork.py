...
# For binary classification, network works with 2 outputs
y_train_oh = np.eye(2)[y_train]
y_test_oh = np.eye(2)[y_test]

# Create the neural network
nn = NeuralNetwork([
    Layer_Dense(X_train.shape[1], 16),   # hidden layer with 16 neurons, inputs are n_features
    Activation_ReLU(),
    Layer_Dense(16, 2),                  # output layer with 2 neurons (binary)
    Activation_Softmax_Loss_CategoricalCrossEntropy()  # combined activation + loss
])

# Train the network
optimizer = Optimizer_Adam(learning_rate=0.01)
nn.train(X_train, y_train_oh, epochs=200, batch_size=16, optimizer=optimizer, verbose=True, X_val=X_test, y_val=y_test_oh)
